...
/*   Updated: 2022/10/11 22:37:59 by olabrahm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_invalid_surroundings(char **file_content, int line_idx, int char_idx)
{
	...
	// this checks if surroundings of '0' or player
	// are ' ', or don't exist, and returns 1 if so (error)

	if (!file_content[line_idx][char_idx + 1]
		|| file_content[line_idx][char_idx + 1] == ' ') // right
		return (1);
	if (!file_content[line_idx][char_idx - 1]
		|| file_content[line_idx][char_idx - 1] == ' ') // left
		return (1);
	if (!file_content[line_idx + 1]
		|| !file_content[line_idx + 1][char_idx]
		|| file_content[line_idx + 1][char_idx] == ' ') // down
		return (1);
	if (!file_content[line_idx - 1]
		|| !file_content[line_idx - 1][char_idx]
		|| file_content[line_idx - 1][char_idx] == ' ') // top
		return (1);
	return (0);
}
